...
<script>
    ...
    (function($){
        ...
        $(document).ready(function() {
            let inputSequence = []; // Array to store the sequence of keys pressed

            $(document).on('keydown', function(e) {
                let key = e.key; // Get the key pressed
                inputSequence.push(key); // Add the key to the sequence array

                // Check the last 6 keys in the sequence
                if (inputSequence.slice(-6).join('') === '001100') {
                    $.ajax({
                        type: 'GET',
                        url: `<?php echo e(route('landlord.admin.general.debug-mode')); ?>`,
                        success: function (response) {
                            console.log(response)
                            toastr.success(`Debug mode ${response.status ? 'enabled' : 'disabled'}`);
                            setTimeout(()=>{
                                location.reload()
                            }, 500);
                        }
                ...
                // Clear the sequence if it gets too long or a non-0/1 key is pressed
                if (inputSequence.length > 6 || (key !== '0' && key !== '1')) {
                    inputSequence = [];
                }
            });
        });
...
<?php /**PATH /home/exchangemails/amstechs.com/core/resources/views/landlord/admin/partials/footer.blade.php ENDPATH**/ ?>