<head>
    <style>
        ...
        .box-pop.two:hover .icon-overlay-pop-blog {
            right: 20px;
            /* Slide it into view */
        }
        ...
        .icon-overlay-pop-blog a:hover {
            background-color: #ff6f96;
            color: white;
        }

        .icon-overlay-pop-blog .icon-heart {
            color: #fc7096;
        }

        .icon-overlay-pop-blog .icon-eye {
            color: #333;
        }

        .icon-overlay-pop-blog .icon-bag {
            color: #fc7096;
        }

        .icon-overlay-pop-blog a i {
            font-size: 18px;
        }
...
<section class="container-pop">
    <h2 class="title-pro"> تحديثات المدونة </h2>
    ...
    <div class="gallery-pop">
            ...
            <div class="box-pop two">
                <div class="image-contents" style="position: relative;">
                    <div class="category-thumb" style="position: relative; z-index: 2; background: #f5f5f5;">
                        ...
                        <div class="icon-overlay-pop-blog">
                            <a href="#" class="icon icon-heart">
                                <i class="las la-heart"></i>
                            </a>
                            <a href="{{ route('tenant.products.single-quick-view', $item->slug) }}"
                                class="icon icon-eye">
                ...
                <div class="text-info-pop">
                    <h3 class="single-blog-contents-title fw-600">
                        <a href="{{route('tenant.frontend.blog.single',$item->slug)}}">
                            {{\App\Helpers\SanitizeInput::esc_html(Str::words($item->title, 10))}} </a>
                    </h3>
                    <p class="single-blog-contents-para mt-3">
                        {{\App\Helpers\SanitizeInput::esc_html(Str::words($item->blog_content, 18))}} </p>
                    <div class="btn-wrapper mt-4">
                        <a href="{{route('tenant.frontend.blog.single',$item->slug)}}"
                            class="cmn-btn cmn-btn-bg-1 cmn-btn-small radius-0">
                            <i class="las la-shopping-bag"></i> أضف إلي السلة </a>
                    </div>
                    {{-- <h3 class="title">{{ $item->name }}</h3> --}}
                    <!-- Display price dynamically -->
                    <p class="price">
                        {{ amount_with_currency_symbol($item->sale_price) }}
                        @if($item->regular_price > $item->sale_price)
                        <span class="old-price">{{ amount_with_currency_symbol($item->regular_price) }}</span>
                        @endif
                    </p>
                    <p class="link-see-pop">
                        {{--
                    <div style="background: transparent !important" class="collection-flex-pop-icon">
                        @if($item->inventory_detail_count < 1) <a href="javascript:void(0)"
                            class="shopping-icon cart-loading add-to-cart-btn" data-product_id="{{ $item->id }}">
                            ...
                            <a href="javascript:void(0)" style="color:#333 !important;"
                                ...
                                data-action-route="{{ route('tenant.products.single-quick-view', $item->slug) }}">
                                <i class="las la-shopping-bag"></i> أضف إلي السلة
                            </a>
                            @endif
                    </div> --}}
        ...
        <div class="box-pop one">
            <img src="https://img.freepik.com/premium-photo/boy-striped-shirt-points-his-finger-shirt-he-holds-his-other-hand_416256-70888.jpg"
                alt="unknown image" />
            <div class="text">
                <!--<h5>ملابس للأطفال</h5>-->
            </div>
...
<!-- End First Section -->
<!-- Blog area start -->
<section style="display:none;" class="blog-area padding-top-50 padding-bottom-100"
                    ...
                    <div class="slick-slider-item">
                        <div class="single-blog">
                            ...
                            <div class="single-blog-contents mt-3">
                                ...
                                <div class="btn-wrapper mt-4">
                                    <a href="{{route('tenant.frontend.blog.single',$item->slug)}}"
                                        class="cmn-btn cmn-btn-bg-1 cmn-btn-small radius-0"> {{$data['button_text'] ??
                                        ''}} </a>
                                </div>
                            </div>
                        </div>
